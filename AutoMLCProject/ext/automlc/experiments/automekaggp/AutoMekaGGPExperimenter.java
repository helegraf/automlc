...
public class AutoMekaGGPExperimenter implements IExperimentSetEvaluator {
	private static final AutoMekaGGPExperimenterConfig CONFIG = ConfigCache.getOrCreate(AutoMekaGGPExperimenterConfig.class);
	...
	public void evaluate(final ExperimentDBEntry experimentEntry, final IExperimentIntermediateResultProcessor processor) throws ExperimentEvaluationFailedException {
		...
		try (SQLAdapter adapter = new SQLAdapter(CONFIG.getDBHost(), CONFIG.getDBUsername(), CONFIG.getDBPassword(), CONFIG.getDBTableName())) {
			...
			// Prepare connection
			ResultsDBConnection connection = new ResultsDBConnection("intermediate_measurements", "final_measurements", "ordered_metric", experimentEntry.getId(), "AutoMekaGGP", adapter);
			...
			AutoMEKA_GGP autoMekaGGP = new AutoMEKA_GGP(new String[] { "-t", trainingTempFile.getAbsolutePath(), "-T", testingTempFile.getAbsolutePath() }, xmlPathSB.toString());
	...
	private void evaluateMLClassifier(final Instances train, final ResultsDBConnection connection, final AutoMEKA_GGP classifier) throws Exception {
	...
	public static void main(final String[] args) throws ExperimentDBInteractionFailedException, IllegalExperimentSetupException {
		ExperimentRunner runner = new ExperimentRunner(CONFIG, new AutoMekaGGPExperimenter(), new ExperimenterSQLHandle(CONFIG));
